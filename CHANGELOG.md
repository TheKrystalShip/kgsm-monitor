...
`/component`: this leaf's configuration (read and apply), its unit, its journal and its commands, the
same answers every component gives. The socket is its own so a node's API finds it from the component's
id alone, and it carries the scrape socket's mode, which is the boundary on both. Two new settings name
it and the override file a change is written to (`Monitor__SurfaceSocketPath`,
`Monitor__ConfigOverridePath`); the override file is the one the unit's drop-in already loads.
