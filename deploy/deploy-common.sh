#!/usr/bin/env bash
#
# deploy-common.sh — the shared parameter block + helpers for kgsm-monitor's deploy scripts.
#
...
# contract. This copy is vendored so a standalone kgsm-monitor clone deploys with no umbrella
...
DEPLOY_USER="${KGSM_MONITOR_USER:-${KGSM_DEPLOY_USER:-$(id -un)}}"
DEPLOY_GROUP="${KGSM_MONITOR_GROUP:-${KGSM_DEPLOY_GROUP:-$(id -gn)}}"

# ── PROJECT BLOCK — the only part that changes per repo ───────────────────────
PROJECT="kgsm-monitor"

UNITS=("kgsm-monitor.service")
ENABLE_UNITS=("kgsm-monitor.service")
