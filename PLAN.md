...
> cgroup, summing CPU/RSS/IO; gated single scan, PID-recycle guard, 60 tests,
> AOT-proven live (busy native server read `cpuPctCore≈100`). See §11 Validation log.
...
- [x] `ProcTreeSampler`: `.pid` → root PID → invert `/proc/*/stat` `ppid` links → BFS the subtree → sum `utime+stime` (→ `cpuPctCore`), RSS (`statm` resident × page size), `/proc/[pid]/io` `read_bytes`/`write_bytes`. Emitted as `kind:"native"` alongside the cgroup servers (the two samplers cover disjoint kinds, so `ServerSampler.Sample()` concatenates). Routed by the resolver's `Kind=="native"` so a broken container isn't misrouted here.
