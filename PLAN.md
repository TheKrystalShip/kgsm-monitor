...
- **Events** (`kgsm/docs/events.md`): KGSM appends one compact JSON line per event to a date-named segment under `event_journal_dir` (default `/var/lib/kgsm/events`) and delivers to nobody — consumers tail the files holding their own `(segment, byte offset)` cursor, with no registration and no exclusive binding. The payload carries `InstanceName`, **not** PID/cgroup → a resolve step is always needed. Events **only fire via `kgsm.sh`**, so an out-of-band change is invisible to them → the journal is durable but still not the watch-list's authority.
...
- **Engine-event history splits record from index.** The journal is the record (durable, engine-owned, pruned on age); the monitor's `events.db` is a derived index that exists to make it queryable, and `POST /events/rebuild` reconstructs it from the surviving segments. Journal retention must stay ≥ `KGSM_MONITOR_EVENT_RETENTION_DAYS`, checked at startup.
